// client-two-side.js - Handles two-sided products for client art application
...
export function initialize(state, elements, ctx) {
    console.log("Initializing two-sided product mode for client");
    ...
    // Base Images - both front and back for two-sided products
    const baseImages = { front: new Image(), back: new Image() };
    const imagesLoaded = { front: false, back: false };
    
    // Preload base images
    if (state.product?.id) {
        preloadBaseImages(state.product.id, baseImages, imagesLoaded);
    }
...
export function preloadBaseImages(productId, baseImages, imagesLoaded) {
    ...
    console.log(`Loading two-sided base images for product: ${productId}`);
    
    const handleLoad = (side) => {
        imagesLoaded[side] = true;
        console.log(`${side} base image loaded successfully`);
        
        // Update canvas size and render if this is the current side
        if (moduleState && moduleState.side === side) {
            updateCanvasSize(baseImages[side], moduleElements);
            if (moduleCtx) {
    ...
    baseImages.front.onerror = () => console.error('Failed to load front base image');
    baseImages.back.onerror = () => console.error('Failed to load back base image');
    
    // Two-sided products use separate front/back images
    baseImages.front.src = `images/flatlays/${productId}-base-front.png`;
    baseImages.back.src = `images/flatlays/${productId}-base-back.png`;
}

export function renderCanvas(state, elements, ctx, baseImages, imagesLoaded) {
    if (!state.selectedVariant || !imagesLoaded[state.side]) {
        console.log("Cannot render: missing variant or image not loaded");
        return;
    }
    
    console.log("Rendering two-sided canvas for side:", state.side);
    ...
    if (printArea) {
        ...
        console.log("No print area found for product:", state.product?.id, "side:", state.side);
    }
...
export function handleSideSwitch(newSide, state, elements, baseImages, imagesLoaded) {
    if (state.side === newSide) return state.side;
    
    console.log("Switching side from", state.side, "to", newSide);
    state.side = newSide;
    ...
    if (imagesLoaded[newSide]) {
        ...
        // Trigger re-render
        if (moduleCtx) {
            renderCanvas(state, elements, moduleCtx, baseImages, imagesLoaded);
        }
    }
...
export function initializeOverlayPosition(state, elements) {
    ...
    console.log("Initializing overlay position for two-sided product");
